Shader "Ocean Toolkit/Ocean Shader"
{
	Properties
	{
		...
		_FogColor("Fog Color", Color) = (1.0, 0.0, 0.0, 1.0)
		_FogStart("Fog Start", Float) = 0.0
		_FogEnd("Fog End", Float) = 1.0
		_FogDensity("Fog Density", Float) = 1.0
		_SpecularRoughness("SpecularRoughness", Float) = 1.0
		...
		SubShader
		{
			...
			#pragma shader_feature OT_UNDER_SIDE
			...
			float3 refl = float3(0.0, 1.0, 1.0);
				...
				float2 refrUv = uv + normal.xz * ot_RefrNormalOffset * saturate(refrDepthBelowSurface / ot_RefrNormalOffsetRamp);
				refrDepthBelowSurface = sampleDepth(refrUv) * _ProjectionParams.z - screenZ;
				...
				//causticsCol = tex2D(ot_NormalMap0, uv).xyz;
				//return float4(causticsCol, 1);
				// ----
				// Foam
				// ----
				...
				fresnel = 1.0;
				#else
				fresnel = pow(1.0 - viewDotNormal, ot_FresnelPow)*(saturate(_WorldSpaceCameraPos.y - input.worldPos.y) + 0.025);
				#endif
				#endif

				sun = ReflectedSunRadianceNice(-viewDir, normal, ot_LightDir, fresnel);

				float dis = distance(input.worldPos, _WorldSpaceCameraPos);

				float dist = (dis - _FogStart)*saturate(input.worldPos.y - _WorldSpaceCameraPos.y) / _FogEnd;
				float fogDensity = saturate(dist * _FogDensity);

				float3 color = (1.0 - foam) * (fresnel * ot_ReflStrength * refl + (1.0 - fresnel) * ot_RefrStrength * refr) + foam.xxx;
...
//#endif
				return float4(color + sun * ot_SunColor.rgb, alpha);
			}
			ENDCG
		}
		}
